...
namespace ShoppingCartService
{
  ...
  internal sealed class ShoppingCartService : StatefulService, IShoppingCartService
  {
    ...
    protected override IEnumerable<ServiceReplicaListener> CreateServiceReplicaListeners()
    {
      return new []
      {
        new ServiceReplicaListener(i => new WcfCommunicationListener<IShoppingCartService>(i, this, this.CreateListenBinding(), "ServiceEndpoint")) 
      };
    }
    ...
    public async Task<List<ShoppingCartItem>> GetItems()
    {
      ...
      using(var tx = this.StateManager.CreateTransaction())
      {
        return (await cart.CreateEnumerableAsync(tx)).ToEnumerable().Select(x => x.Value).ToList();
      }
    }
